...
def intersect_color(origin, dir, intensity):
    ...
    if (min_distance == np.inf) or (intensity < 0.01): # 最近的交点没有发生交点，或者点
        return np.array([0., 0., 0.]) # 交点太远了，则全部设置为0

    obj = scene[obj_index]
    P = origin + dir * min_distance     # 交点坐标
    color = get_color(obj, P) # 环境光与物体颜色反射
    N = get_normal(obj, P)                  # 交点处单位法向量
    PL = normalize(light_point - P) # 接收光线的坐标
    PO = normalize(origin - P) # 预测光线的坐标

    c = ambient * color # 计算环境光颜色
...
for i, x in track(enumerate(np.linspace(S[0], S[2], w))): # 遍历屏幕每一个像素点 加进度条跟踪
...
time_c = time_end - time_start # 计算消耗时间
time_c = round(time_c,3) # 精确到小数点后三位

plt.imsave('test.png', img) # 将屏幕图像保存为图片
print("本次光追计算消耗 ",time_c," 秒")